...
`publish_report` is a backend-grounded closeout instead of a model-authored row write. The model supplies report fields and evidence `referenceId` values, but the backend validates the report shape, rejects non-citable or out-of-attempt references, derives `is_mass_issue` from the job-level `NeighborSet`, derives evidence kind from artifact state and `memory_items.kind`, and persists `triage_reports`, `triage_evidence`, job/fault terminal state and `ReportPublished` in one transaction. `WorkerOutput` artifacts are never citable. The arguments must use exactly one envelope (`report_json` alone, `report` alone, or a bare report with neither), and `summary` (4000), `recommendedNextAction` (2000), `limitations` (20 items of 1000 characters), `evidence` (50 items) and `quote` (1000) are bounded; the limits are stated in the tool description rather than as schema keywords, and a value past one is refused, never truncated, with a reprompt that names the violated rule. `GET /api/v1/triage-reports/{id}` returns the report and grounded evidence, including the cited artifact payload.
